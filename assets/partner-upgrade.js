...
import {applyPartnerMembership} from './partner-membership.mjs';
export async function openPremiumUpgrade(){
  ...
  const broker=profile.tradeId==='BROKER',files=broker?'Verkaufsakten':'Gewerkakten';
...
export async function mountPremiumPromotion(){
  ...
  const profile=await portalRequest('/api/partner-basic/profile');if(!page.isConnected)return;applyPartnerMembership(profile);if(profile.referralOnly||profile.plan==='premium'||page.querySelector('.basic-premium-promotion'))return;
  ...
  const banner=document.createElement('section');banner.className='card basic-premium-promotion';banner.style.marginBottom='18px';banner.innerHTML=`<div class="toolbar"><div><b>Sie nutzen Basic</b><p>Unbegrenzt empfehlen · bis zu 3 ${profile.tradeId==='BROKER'?'Verkaufsakten':'Gewerkakten'} betreuen. Mehr Möglichkeiten mit Premium.</p></div><button class="primary">Premium entdecken →</button></div>`;banner.querySelector('button').onclick=openPremiumUpgrade;page.prepend(banner);
}
