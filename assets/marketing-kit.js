...
export function previewMarketingFile(result){
  ...
  else{const note=document.createElement('p');note.textContent='Die PDF wird direkt im PDF-Viewer Ihres Browsers geöffnet. So funktioniert die Ansicht auch auf Mobilgeräten und ohne blockierte Einbettung.';box.append(note);const link=document.createElement('a');link.href=result.url;link.target='_blank';link.rel='noopener noreferrer';link.className='primary';link.textContent='PDF ansehen – in neuem Tab';box.append(link);}
}
